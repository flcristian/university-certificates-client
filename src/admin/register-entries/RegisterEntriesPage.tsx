...
import type { CertificateTemplate } from "../../schemas/CertificateTemplateSchema"
import type { ColumnDef } from "@tanstack/react-table"
import { DataTable } from "../../components/ui/data-table"
import { getAllRegisterEntries, updateRegisterEntry } from "../../services/RegisterEntriesService"
import type {
  AcceptRegisterEntryRequest,
  DenyRegisterEntryRequest,
  RegisterEntry,
} from "../../schemas/RegisterEntrySchema"
...
import { getRegisterEntryStateValues, RegisterEntryState } from "../../enums/RegisterEntryState"
import { RegisterEntryFilter } from "./RegisterEntriesFilter"
...
export default function RegisterEntriesPage() {
  ...
  // Apply filters when selected states change
  useEffect(() => {
    ...
    const filtered = registerEntries.filter((entry) => {
      return selectedStates.some((state) => {
        const stateValues = getRegisterEntryStateValues(state)
        return entry.reviewed === stateValues.reviewed && entry.accepted === stateValues.accepted
      })
    })

    setFilteredEntries(filtered)
  }, [selectedStates, registerEntries])

  const handleFilterChange = (states: RegisterEntryState[]) => {
      ...
      cell: ({ row }) => {
        ...
        return (
          <div className="flex flex-col gap-2">
            ...
            {reviewed && accepted && <Button variant="secondary">Accepted</Button>}
            {reviewed && !accepted && (
    ...
    <div className="flex flex-col gap-4 min-h-screen w-[1200px]">
      ...
      {!error && !isLoading && (
        <div className="w-full">
          <div className="flex justify-between items-center mb-4">
            <RegisterEntryFilter onFilterChange={handleFilterChange} />
          </div>
      ...
      {currentForm !== "none" && (
        <div className="relative w-fit">
          <h1
            className="absolute text-4xl text-white cursor-pointer right-4 top-1"
            ...
            <Card className="bg-zinc-900">
              ...
              <CardContent>
                <div className="flex items-center gap-2 text-zinc-100">
                  <h1>Name:</h1>
                  <h1>
                    {selectedRegisterEntry!.student.firstName} {selectedRegisterEntry!.student.lastName}
                  </h1>
                </div>
                <div className="flex items-center gap-2 text-zinc-100">
                  <h1>Email:</h1>
          ...
          <DialogFooter>
            ...
            <div>
              <Popover open={acceptPopupOpen} onOpenChange={(isOpen) => setAcceptPopupOpen(isOpen)}>
                ...
                <PopoverContent className="bg-zinc-900 border-zinc-600">
                  <div className="flex flex-col gap-2">
                    <h1 className="text-zinc-100">Are you sure you want to accept this request?</h1>
                    <div className="flex items-center gap-2">
                      <Button
                        variant="secondary"
                        onClick={() => {
